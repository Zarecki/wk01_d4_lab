...
class TestFriends < MiniTest::Test
  ...
  def test_add_friend
    initial_number_of_friends = @person3[:friends].length
    add_friend(@person3)
    assert_equal(initial_number_of_friends + 1, @person3[:friends].length)
  end

  # 5. For a given person, remove a specific name from their list of friends
  # (hint: Same as above, testing for the length of the array should be sufficient)
  def test_remove_friend
     initial_number_of_pals = @person4[:friends].length
     remove_friend(@person4)
     assert_equal(initial_number_of_pals - 1, @person4[:friends].length)
  end
  ...
  def test_loan
      lender_money = @person1[:monies]
      lendee_money = @person2[:monies]
      loan(@person1, @person2, 1)
      assert_equal(lender_money - 1, @person1[:monies])
      assert_equal(lendee_money + 1, @person2[:monies])
  end
  # 8. Find the set of everyone's favourite food joined together
  # (hint: concatenate the favourites/snack arrays together)
def test_favourite_food
    @people = [@person1, @person2, @person3, @person4, @person5]
    favourite_food(@people)
    result = menu
    assert_equal(menu.length, 7)
end
